...
int bootenv_property_list(void (*propfn)(const char *key, const char *value, void *cookie), void *cookie)
{
#if 0
...
int write_args_to_file()
{
    ...
    memset((void*)&burn_args_data,0,sizeof(burn_args_data));
    sprintf(burn_args_data.data , "%s\n%s\n%s\n", bootenv_get_value(ARGS_ETHADDR),
            bootenv_get_value(ARGS_LICENCE),bootenv_get_value(ARGS_DEVICE_ID)) ;
...
#endif /* BOOT_ARGS_CHECK */

/* Power Manager Wrapper Functions */
bool pmInit(void)
{
    /* bootenv_init() returns 0 on success. */
    return (bootenv_init()? false : true);
}
...
bool pmReadPowerState(uint32_t *pmState)
{
    ...
    char Buffer[16] = {'\0'};
    if (pmState) {
        strncpy(Buffer, bootenv_get("lastpwrmode"), (sizeof(Buffer)-1));
        *pmState = (uint32_t)atoi(Buffer);
        status = true;
    }
    return status;
}
...
bool pmSavePowerState(uint32_t pmState)
{
    bool status = false;
    char buffer[16] = {'\0'};
    snprintf(buffer, (sizeof(buffer)-1), "%u", pmState);
    /* bootenv_update() returns 0 on success. */
    status = ((bootenv_update("lastpwrmode", buffer))? false : true);
    return status;
}
